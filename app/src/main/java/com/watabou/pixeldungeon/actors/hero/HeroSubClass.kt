...
enum class HeroSubClass(private val title: String?, private val desc: String?) {
    NONE(null, null),
    GLADIATOR(
        "gladiator",
    ...
    fun storeInBundle(bundle: Bundle) {
        bundle.put(TAG_SUBCLASS, toString())
    }
    companion object {
        private const val TAG_SUBCLASS = "subClass"
        fun restoreInBundle(bundle: Bundle): HeroSubClass {
            val value = bundle.getString(TAG_SUBCLASS)
            return try {
                valueOf(value)
            } catch (e: Exception) {
                NONE
